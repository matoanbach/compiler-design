...
- If all th eright-hand sides begin with terminal symbols, the choice is straightforward.
- If some right-hand sides begin with non-terminals, the parser must know what teoken can begin any sequence generated by this non-terminal (i.e the FIRST set of this non-terminal)
- If a FIRST set contains epsilon (i.e. the non-terminal can be "absent" by using an epsilon-production), it must know what may follow this non-terminal (i.e. the FOLLOW set of this non-terminal) in order to chose an epsilon-production
